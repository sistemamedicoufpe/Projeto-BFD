...
function loadPatientData() {
    ...
    // Preencher a grid de informações
    document.getElementById('infoBirthDate').textContent = patientData.birthDate;
    document.getElementById('infoGender').textContent = patientData.gender;
    document.getElementById('infoContact').textContent = patientData.contact;
    document.getElementById('infoResponsible').textContent = patientData.responsible;
    document.getElementById('infoFamilyHistory').textContent = patientData.familyHistory;
    document.getElementById('infoComorbidities').textContent = patientData.comorbidities;
    ...
    document.querySelector('.risk-details h4').textContent = `Risco de Demência: ${patientData.riskLevel}`;
    document.querySelector('.risk-details p').textContent = patientData.riskDescription;
...
function setupModal() {
    ...
    const openModalButtons = document.querySelectorAll('.open-modal'); // Não temos esse botão, vamos adicionar?
    const closeModalButtons = document.querySelectorAll('.close-modal');

    // Vamos adicionar um botão para abrir o modal no header do paciente
...
Agora, note que no HTML original não temos o canvas para o gráfico. Vamos substituir a div do gráfico no HTML por:

html
<div class="chart-container">
    <canvas id="progressChart"></canvas>
</div>
Além disso, adicionei um botão "Editar" no patient-header via JavaScript.

Agora, a versão TypeScript (script.ts):

Vamos criar uma versão TypeScript com tipagem básica.
